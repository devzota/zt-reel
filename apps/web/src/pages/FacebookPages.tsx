...
function FanpageRow({ page, isExpired, testingPageId, handleTestPost, handleToggleActive, isSelected, onToggleSelect }: any) {
...
await ztteam_deletePage(page.id);
  ...
  return (
    ...
    <tr>
        <td colSpan={7} className="px-6 pb-6 pt-2 border-b border-slate-100">
...
function FanpageMobileCard({ page, isExpired, testingPageId, handleTestPost, handleToggleActive, isSelected, onToggleSelect }: any) {
  const navigate = useNavigate();
  const { ztteam_getPageReport, ztteam_deletePage } = useZTTeamFacebookStore();
  const { ztteam_showToast, ztteam_showConfirm } = useUIStore();
  const [insights, setInsights] = useState<any>(null);
